...
export async function redeemInviteAction(input: RedeemInviteInput) {
  try {
    ...
    // Antes: erro era só logado (console.error) e engolido — o convite era
    // marcado como aceito mesmo se esse vínculo falhasse, sem ninguém saber.
    // Agora: qualquer falha aqui interrompe o fluxo e aparece como erro real
    // na tela (via toast, no onError da mutation no client).
    if (inviteCourses.length) {
      ...
      if (courseErr) {
        console.error("Erro ao salvar membership_courses:", courseErr);
        throw new Error(`Falha ao vincular curso(s): ${courseErr.message}`);
      }
    }

    // 3. Vínculo com Polos (membership_polos)
    // Mesma correção aplicada aqui por consistência, embora este já
    // estivesse funcionando na prática.
    if (finalPolos.length) {
      ...
      if (poloErr) {
        console.error("Erro ao salvar membership_polos:", poloErr);
        throw new Error(`Falha ao vincular polo(s): ${poloErr.message}`);
      }
    }
