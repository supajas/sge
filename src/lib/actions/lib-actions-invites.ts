...
import { roleRequiresPolo, roleRequiresCourse } from "../invite-requirements";
...
export async function previewInviteAction(code: string) {
  ...
  // Checa se o papel exige e se já possui polos vinculados (fonte única: invite-requirements.ts)
  const requiresPolo = roleRequiresPolo(role);
...
export async function redeemInviteAction(input: RedeemInput) {
  ...
  // Validação de polo/curso pela mesma fonte única usada na criação do
  // convite e no outro fluxo de resgate (invite/[code]/actions.ts).
  if (roleRequiresPolo(finalRole) && finalPolos.length === 0) {
    throw new Error("Selecione ao menos um polo para continuar");
  }
  if (roleRequiresCourse(finalRole) && inviteCourses.length === 0) {
    throw new Error(
      "Este convite está sem curso vinculado. Peça ao administrador para editar o convite e selecionar ao menos um curso."
    );
  }
  ...
  // Vínculo com Cursos (Professor, Coordenador de Curso, Tutor a Distância, Tutor Presencial)
  // NOTA: a tabela atual é `membership_courses` — a antiga `coordinator_courses`
  // foi substituída (ver migration de RLS). Escrever no nome antigo faz o
  // vínculo "sumir" para o resto do app, que já lê só de membership_courses.
  if (inviteCourses.length) {
    const { error: courseErr } = await supabaseAdmin.from("membership_courses").upsert(
      inviteCourses.map((cid: string) => ({
        membership_id: membershipId!,
        course_id: cid,
      })),
      { onConflict: "membership_id,course_id" },
    );
    if (courseErr) throw new Error(`Falha ao vincular curso(s): ${courseErr.message}`);
  }

  // Vínculo com Polos (Coordenador de Polo, Tutor Presencial, Professor, Tutor a Distância)
  // Mesma observação acima: tabela atual é `membership_polos`, não `coordinator_polos`.
  if (finalPolos.length) {
    const { error: upsertError } = await supabaseAdmin.from("membership_polos").upsert(
  ...
  const { data: membershipsData, error: membershipsError } = await supabase
    .from("memberships")
    .select("id, role, institution_id, institutions!inner(name, city, state, logo_url), membership_polos(polo_id)")
    .eq("user_id", user.id);
